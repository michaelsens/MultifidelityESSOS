...
def multifidelity_loss(x):
    global bias, first_iter, idxs, initial_values
    dofs, currents = jax.lax.cond(
        change_currents,
        lambda _: (jnp.reshape(x[:len_dofs], shape=stel.dofs.shape), x[-n_curves:]),
        lambda _: (jnp.reshape(x, shape=stel.dofs.shape), stel.currents[:n_curves]),
    ...
    np.random.seed(iteration)  # Add randomization for consistency
    random_offsets = np.random.normal(0, 0.01, initial_values.shape)
    randomized_initial_values = initial_values + random_offsets

    if first_iter:
        high_fidelity_value, particle_losses = high_fidelity_loss(dofs, currents, stel, particles, R, r_init, randomized_initial_values, maxtime, timesteps, n_segments, model)

        idxs = jnp.argsort(particle_losses)[-nparticles // 3:]
        print(idxs)
        low_fidelity_value = low_fidelity_loss(dofs, currents, stel, R, r_init, randomized_initial_values[:, idxs], maxtime, timesteps, n_segments, model)
...
def callback(x, res=None):
    global flag_value, iteration, bias, objective_value, initial_values
