...
Delta = 1.0   # Initial trust region size
Delta_star = 10.0  # Maximum trust region size
eta1, eta2 = 0.5, 2.0  # Shrinking and expanding factors
gamma1, gamma2 = 0.25, 0.75  # Thresholds for shrinking and expanding
global high_fidelity_value, low_fidelity_value, gamma

# Modify multifidelity_loss to track low-fidelity loss with bias
def multifidelity_loss(x):
    global bias, first_iter, idxs, high_fidelity_value, low_fidelity_value
    dofs, currents = jax.lax.cond(
        change_currents,
        lambda _: (jnp.reshape(x[:len_dofs], shape=stel.dofs.shape), x[-n_curves:]),
        lambda _: (jnp.reshape(x, shape=stel.dofs.shape), stel.currents[:n_curves]),
    ...
    # First iteration setup for high-fidelity value and bias
    if first_iter:
        ...
        # Only low-fidelity loss + bias in subsequent iterations
        low_fidelity_value = low_fidelity_loss(dofs, currents, stel, R, r_init, initial_values[:,idxs], maxtime, timesteps, n_segments, model)
    
    return low_fidelity_value + bias

# Callback function to update trust region parameters after each iteration
def callback(x):
    global Delta, iteration, bias, objective_value, high_fidelity_value, low_fidelity_value, gamma

    # Compute new high-fidelity loss and determine gamma
    dofs, currents = jax.lax.cond(
        change_currents,
        lambda _: (jnp.reshape(x[:len_dofs], shape=stel.dofs.shape), x[-n_curves:]),
        lambda _: (jnp.reshape(x, shape=stel.dofs.shape), stel.currents[:n_curves]),
    ...
    # Get updated high-fidelity loss and particle losses
    new_high_fidelity_value, particle_losses = high_fidelity_loss(dofs, currents, stel, particles, R, r_init, initial_values, maxtime, timesteps, n_segments, model)
    idxs = jnp.argsort(particle_losses)[-nparticles // 3:]
    
    # Calculate gamma for trust-region adjustment
    if high_fidelity_value is not None:
        gamma = (high_fidelity_value - new_high_fidelity_value) / (high_fidelity_value - low_fidelity_value)
        high_fidelity_value = new_high_fidelity_value  # Update for the next iteration
    else:
        gamma = 1  # Initialize gamma on the first iteration

    # Adjust trust region size Delta based on gamma
    if gamma < gamma1:
        Delta = eta1 * Delta
    elif gamma > gamma2:
        Delta = min(eta2 * Delta, Delta_star)
    else:
        Delta = jnp.linalg.norm(x - jnp.array(objective_value[-1]))  # Current step size

    # Update bias based on high-fidelity and low-fidelity loss difference
    low_fidelity_value = low_fidelity_loss(dofs, currents, stel, R, r_init, initial_values[:, idxs], maxtime, timesteps, n_segments, model)
    bias = high_fidelity_value - low_fidelity_value

    # Log the results of this iteration
    objective_value.append(new_high_fidelity_value)
    print(f"Iteration {iteration}: HF Loss {new_high_fidelity_value}, LF Loss {low_fidelity_value}, Bias {bias}, Delta {Delta}, Gamma {gamma}")
