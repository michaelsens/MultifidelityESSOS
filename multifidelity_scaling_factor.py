...
def multifidelity_loss(x):
    global scaling_factor, first_iter, idxs
    dofs, currents = jax.lax.cond(
        change_currents,
        lambda _: (jnp.reshape(x[:len_dofs], shape=stel.dofs.shape), x[-n_curves:]),
        lambda _: (jnp.reshape(x, shape=stel.dofs.shape), stel.currents[:n_curves]),
    ...
    if first_iter:
        ...
        scaling_factor = high_fidelity_value / low_fidelity_value

        print(f"HighFidelity Step: HF Loss {high_fidelity_value} --- LF Loss {low_fidelity_value} --- scaling_factor {scaling_factor}")
        first_iter = False
    else:
        low_fidelity_value = low_fidelity_loss(dofs, currents, stel, R, r_init, initial_values[:,idxs], maxtime, timesteps, n_segments, model)
        #print(f"LF Loss {low_fidelity_value} --- scaling_factor {scaling_factor} --- Loss: {low_fidelity_value + bias}")
    
    return low_fidelity_value * scaling_factor

def callback(x):
    global flag_value,iteration, scaling_factor, objective_value
